...
void MenuFactory::create_filament_action_menu(bool init, int active_filament_menu_id)
{
    ...
    for (int i = 0; i < filaments_cnt; i++) {
        ...
        wxString item_name = preset ? from_u8(preset->label(false)) :
                                      wxString::Format(_L("Filament %d"), filament_index_from_zero_based(i));
...
void MenuFactory::create_plate_menu()
{
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Select All"), _L("select all objects on current plate"),
    ...
    append_menu_item(
        menu, wxID_ANY, _L("Delete All"), _L("delete all objects on current plate"),
